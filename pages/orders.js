...
class Tables extends React.Component<Props> {
  ...
  static async getInitialProps(ctx) {
    ...
    try {
      ...
      return {
        orders: orders.orders, branchname: orders.branchName, partnername: orders.partnerName, token, query: ctx.query,
      };
    } catch (e) {
  ...
  async newOrders() {
    ...
    try {
      ...
      if (orders) {
        this.setState({ ordersnew: orders.orders, disabledCancel: '', disabledDelivey: '', disabledSend: '' });
      }
    } catch (e) {
    }
  }
  ...
  ChangeState = (itemId) => {
    this.setState({ disabledCancel: itemId });
  }

  async handleClick(orderId) {
    this.setState({ disabledCancel: orderId });
    const { token } = this.props;
    let txt;
  ...
  async approveBtn(orderId) {
    this.setState({ disabledSend: orderId });
    const { token } = this.props;
  ...
  async delivereBtn(orderId) {
    this.setState({ disabledDelivery: orderId });
    const { token } = this.props;
  ...
  render() {
    ...
    const {
      classes, query, branchname, partnername,
    } = this.props;
    const {
      notificationOn, timealert, ordersnew, isOpen, disabledCancel, disabledDelivery, disabledSend,
    } = this.state;
                        ...
                        <div className={item.status === 5 ? classes.hidden : classes.show}>
                          <div className={item.status === 4 ? classes.hidden : classes.show}>
                            <div className={(item.status === 2 || item.status === 3) ? classes.hidden : classes.show}>
                              ...
                              <Button variant="contained" color="secondary" className={`${classes.reject} ${classes.button}`} onClick={this.handleClick.bind(this, item.id)} disabled={disabledCancel === item.id}>
                                <span>Cancel</span>
                            ...
                            <div className={item.status === 3 ? classes.hidden : classes.show}>
                              ...
                              <div className={item.status === 2 ? classes.hidden : classes.show}>
                                <Button variant="contained" color="primary" className={`${classes.approve} ${classes.button}`} onClick={this.approveBtn.bind(this, item.id)} disabled={disabledSend === item.id}>Send to Gregsys</Button>
                              </div>
                            </div>

                            <div className={item.status === 3 ? classes.hidden : classes.show}>
                              <div className={item.status === 1 ? classes.hidden : classes.show}>
                                <Button variant="contained" color="primary" className={`${classes.delivered} ${classes.button}`} onClick={this.delivereBtn.bind(this, item.id)} disabled={disabledDelivery === item.id}>Delivered </Button>
                              </div>
                            </div>
                          </div>
